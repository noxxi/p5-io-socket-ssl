...
C<SSL_force_fingerprint>.

=item SSL_force_fingerprint

Set this to true to enforce strict certificate pinning: the connection will be
rejected unless the peer certificate matches one of the fingerprints given in
C<SSL_fingerprint>, even if the certificate chain would otherwise be trusted.

Without this option, C<SSL_fingerprint> acts as an alternative trust anchor —
a matching fingerprint can unlock an otherwise-rejected certificate (e.g.
self-signed or from an unknown CA), but a non-matching fingerprint does not
block a certificate that passes normal chain validation. With this option, the
fingerprint check is the sole trust decision: any certificate not in the
fingerprint list is rejected regardless of CA trust.
...
Try OCSP stapling, but don't fail if no usable stapled response is received.
Both a missing staple and a broken or unverifiable staple are treated the same:
the connection continues without OCSP revocation information from the staple.
This is the default if SSL_verify_mode is VERIFY_PEER (the default).

=item SSL_OCSP_MUST_STAPLE

Require a valid stapled OCSP response; fail the connection if none is received.
A broken or unverifiable staple is treated the same as a missing one — both
cause the connection to fail with "did not receive the required stapled OCSP
response". Most servers currently send no stapled OCSP response back.
...
C<<[identity,psk]>>. Alternatively only the psk might be provided, in which case
C<io_socket_ssl> is used as identity.
...
psk based on the identity hint. The special identity C<''> is used to set a
default psk.
...
If this option is used with C<start_SSL> (not supported somewhere else) it will
not upgrade the given IO handle but instead wrap it inside a new IO object and
use openssl memory BIO's to manage exchange between TLS layer and socket layer.
While this will add some overhead it makes it possible to stack TLS layers, i.e.
first create an outer TLS connection (without or with SSL_usebio) and then
create the inner TLS layer with SSL_usebio, using the outer TLS connection as
the IO handle in C<start_SSL>.

Example:

   # create TLS connection to TLS enabled http proxy
   my $outer = IO::Socket::SSL->new('proxy.test:3128');

   # send proxy request
   print $outer "CONNECT real-target.test:443 HTTP/1.0\r\n\r\n";
   ... # read and check response from proxy on $outer

   # create an inner TLS connection to the server inside $outer
   my $inner = IO::Socket::SSL->start_SSL($outer,
      SSL_usebio => 1,
      SSL_hostname => 'real-target.test', # SNI
   );

   # send request to server through proxy using $inner
   print $inner "GET / HTTP/1.0\r\nHost: real-target.test\r\n\r\n";
...
peek() with the same arguments will return the same results.
...
Note that if start_SSL() fails (wrong arguments, error in SSL negotiation ...),
it will try to bless the socket back to its original class. This will fail
if the given socket was not blessed, since there is no builtin "unbless".
Therefore better use IO::Socket (or other IO::Handle) objects as C<$socket>
instead of creating unblessed sockets with C<socket> or similar.

For non-blocking sockets you better just upgrade the socket to IO::Socket::SSL
and call accept_SSL or connect_SSL and the upgraded object. To just upgrade the
socket set B<SSL_startHandshake> explicitly to 0. If you call start_SSL w/o this
parameter it will revert to blocking behavior for accept_SSL and connect_SSL.
...
If C<SSL_fast_shutdown> is set to true only the write direction will be
...
during read or by explicitly calling C<stop_SSL> with C<SSL_fast_shutdown> false.

After a complete stop_SSL the socket can again be used to exchange plain data.
Even if the SSL shutdown is only done for writing (C<SSL_fast_shutdown>) plain
