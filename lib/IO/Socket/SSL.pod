...
that nobody in between can read them. It also provides temper resistance so that
...
trusted root certificates. These are usually stored in a system dependend
...
it might be compromised or the underlying cryptographie got broken in the mean
time.
...
When accessing a web site with SSL or delivering a mail a secure way the
...
This usually works find on UNIX systems.
...
same IP address.
It will also do check the revocation of the certificate with OCSP, but currently
...
Because a client with improper oder slow SSL handshake could make the server
...
plattforms (UNIX, Mac OSX and Windows) it is better to use the defaults provided
by IO::Socket::SSL.
...
Helpful for debugging are a knowledge of SSL interna, wireshark and the use of
...
can often be detected because they support much less ciphers than other
implementations.
...
features and usually less known bugs.
...
and return to the class before start_SSL(). It gets the same arguments as
...
upgraded with C<start_SSL> and C<SSL_startHandshake> 0.
