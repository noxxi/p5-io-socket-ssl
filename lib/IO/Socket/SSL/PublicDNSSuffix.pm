...
use strict;
use warnings;
package IO::Socket::SSL::PublicDNSSuffix;
...
IO::Socket::SSL::PublicDNSSuffix - provide access to Mozillas list of effective TLD names

=head1 SYNOPSIS

    # load builtin list on demand unless load_file/load_data are called
    use IO::Socket::SSL::PublicDNSSuffix;

    # alternatively load list from file immediately
    # use IO::Socket::SSL::PublicDNSSuffix file => 'public_suffix.txt';

    # alternatively load list from data immediately
    # use IO::Socket::SSL::PublicDNSSuffix data => '...';

    # alternatively load default list immediately
    # use IO::Socket::SSL::PublicDNSSuffix 'default';
    ...
    # $rest -> whatever.host
    # $tld  -> co.uk
    my ($rest,$tld) = public_suffix('whatever.host.co.uk');
    my $tld = public_suffix('whatever.host.co.uk');

    # $root_domain -> host.co.uk
    my $root_domain = public_suffix('whatever.host.co.uk', 1);

    # --- array in -> array out
    # $rest -> [qw(whatever host)]
    # $tld  -> [qw(co uk)]
    my ($rest,$tld) = public_suffix([qw(whatever host co uk)]);

 ----
    
    # To update this file with the current list:
    perl -MIO::Socket::SSL::PublicDNS::Suffix -e 'IO::Socket::SSL::PublicDNS::Suffix::_update_from_url()'
...
=item public_suffix( $host|\@host, [ $add ], [ $suffix_data ])
...
The function is exported per default.

If C<$suffix_data> is given it will use it, otherwise it will fall back to a
builtin list. C<$suffix_data> can be either a filename, a scalar reference to a
string definition or the result of C<load_data>.

=item load_data( $file | \$string )

Load public suffix list from file or string reference and return internal
representation which then can be used inside public_suffix.
...
=head1 BUGS
 ...
 Q: Why yet another module, we already have L<Domain::PublicSuffix> and L<Mozilla::PublicSuffix>.
 A: Because the public suffix data change more often than these modules do, IO::Socket::SSL needs
    this list and it is more easy this way to keep it up-to-date.
...
BEGIN {
	...
	*idn_to_unicode = \&URI::_idn_to_unicode;
	...
	*idn_to_unicode = \&Net::LibIDN::idn_to_unicode;
	...
	die "cannot find IDNA handling library";
    }
}
...
sub import {
    shift; # class
    my @arg = @_;
    push @arg, ':DEFAULT' if ! @arg;
    while (@arg) {
	if ( $arg[0] eq 'file' ) {
	    shift(@arg);
	    $builtin_dom_data = load_file( shift(@arg));
	} elsif ( $arg[0] eq 'data' ) {
	    shift(@arg);
	    $builtin_dom_data = load_data( \( shift(@arg)));
	} elsif ( $arg[0] eq 'default' ) {
	    shift(@arg);
	    $builtin_dom_data = load_data( \( _default_data() ));
	} elsif ( $arg[0] eq ':DEFAULT' ) {
	    shift(@arg);
	    my $pkg = (caller)[0];
	    no strict 'refs';
	    *{ "${pkg}::public_suffix" } = \&public_suffix;
	} else {
	    die "unknown argument while loading: $arg[0]";
	}
    }
}

{
    # cache
    my $dom_data;   # hash
    my $dom_source; # [ file,mtime,size ] if source was file
    sub load_data {
	my $data = shift;
	my $fh;
	if ( ref $data ) {
	    ...
	    open( $fh,'<', \( _default_data() ));
	} else {
	    return $dom_data if $dom_data
		&& $dom_source 
		&& $dom_source->[0] eq $data
		&& $dom_source->[1] == (stat($data))[9]
		&& $dom_source->[2] == (stat(_))[7];
	    open( $fh,'<:utf8',$data ) or return;
	}
	if ( $dom_data = _load_fh($fh)) {
	    # cache 
	    $dom_source = ! defined $data || ref($data) 
		? undef 
		: [ $data, (stat($fh))[9,7] ];
	    return $dom_data;
	}
...
sub public_suffix {
    my ($name,$add,$suffix_data) = @_;
    if ( ! $suffix_data ) {
	$suffix_data = $builtin_dom_data ||= load_data();
    } elsif ( ! ref $suffix_data || ref($suffix_data) eq 'SCALAR' ) {
	$suffix_data = load_data( $suffix_data )
    }
	...
	if ( my $px = $p->{$name->[-1]} ) {
	    ...
	    $p = $px;
	    unshift @$public,pop(@$name);
	} elsif ( $p->{ "!$name->[-1]" } ) {
	    # found as exception, keep last and return
	    last;
	} elsif ( $p->{'*'} ) {
	    # match anything and continue
	    $p = $px;
	    unshift @$public,pop(@$name);
	} else {
	    # no match
	...
	$name = join('.',@$name);
	if ( $want eq 'u' ) {
	    $public = idn_to_unicode($public);
	    $name   = idn_to_unicode($name);
	}
    }

    return wantarray ? ($name,$public):$public;
}
